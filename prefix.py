...
class PrefixMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        environ['SCRIPT_NAME'] = self.service_prefix + environ['SERVER_PORT'] if self.service_prefix else ''
        # Strip SCRIPT_NAME from PATH_INFO so Flask sees just the route
        if environ.get('SCRIPT_NAME') and environ.get('PATH_INFO', '').startswith(environ['SCRIPT_NAME']):
